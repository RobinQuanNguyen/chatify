...
function DemoTourOverlay({
  ...
  const safeWidth = containerSize?.width || 960
  const safeHeight = containerSize?.height || 800
  const cardWidth = Math.min(TOUR_CARD_WIDTH, Math.max(260, safeWidth - 32))
    ...
    return (
      <div className="absolute inset-0 z-40">
        ...
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
          <div className="rounded-xl border border-cyan-500/40 bg-slate-900/95 p-4 shadow-2xl shadow-cyan-900/30" style={tooltipBase}>
            <p className="text-xs uppercase tracking-wider text-cyan-400 mb-1">Demo Tour</p>
            ...
            <div className="mt-4 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  className="px-3 py-1.5 rounded-md text-xs text-slate-300 bg-slate-700/70 hover:bg-slate-600/80 transition-colors"
                  onClick={onSkip}
                >
                  Skip
                </button>
                <button
                  type="button"
                  className="px-3 py-1.5 rounded-md text-xs font-medium text-slate-950 bg-cyan-400 hover:bg-cyan-300 transition-colors"
                  onClick={onNext}
                >
                  Got it
                </button>
  ...
  if (placement === "right") {
    tooltipStyle.left = targetRect.right + TOUR_GAP
    tooltipStyle.top = targetRect.top + targetRect.height / 2 - TOUR_CARD_HEIGHT / 2
    arrowClass.wrapper = "absolute left-0 top-1/2 -translate-x-[7px] -translate-y-1/2"
    arrowClass.arrow = "size-3 rotate-45 bg-slate-900 border-l border-b border-cyan-500/40"
  }

  if (placement === "left") {
    tooltipStyle.left = targetRect.left - cardWidth - TOUR_GAP
    tooltipStyle.top = targetRect.top + targetRect.height / 2 - TOUR_CARD_HEIGHT / 2
    arrowClass.wrapper = "absolute right-0 top-1/2 translate-x-[7px] -translate-y-1/2"
    arrowClass.arrow = "size-3 rotate-45 bg-slate-900 border-r border-t border-cyan-500/40"
  }

  if (placement === "top") {
    tooltipStyle.left = targetRect.left + targetRect.width / 2 - cardWidth / 2
    tooltipStyle.top = targetRect.top - TOUR_CARD_HEIGHT - TOUR_GAP
    arrowClass.wrapper = "absolute left-1/2 bottom-0 translate-y-[7px] -translate-x-1/2"
    arrowClass.arrow = "size-3 rotate-45 bg-slate-900 border-r border-b border-cyan-500/40"
  }

  if (placement === "bottom") {
    tooltipStyle.left = targetRect.left + targetRect.width / 2 - cardWidth / 2
    tooltipStyle.top = targetRect.bottom + TOUR_GAP
    arrowClass.wrapper = "absolute left-1/2 top-0 -translate-y-[7px] -translate-x-1/2"
    arrowClass.arrow = "size-3 rotate-45 bg-slate-900 border-l border-t border-cyan-500/40"
  }

  tooltipStyle.left = clamp(tooltipStyle.left, 16, safeWidth - cardWidth - 16)
  tooltipStyle.top = clamp(tooltipStyle.top, 16, safeHeight - TOUR_CARD_HEIGHT - 16)
  ...
  return (
    <div className="absolute inset-0 z-40">
      ...
      <div
        className="absolute rounded-xl ring-2 ring-cyan-400/80 shadow-[0_0_0_999px_rgba(2,6,23,0.35)] transition-all duration-200"
        style={{
      ...
      <div
        className="absolute rounded-xl border border-cyan-500/40 bg-slate-900/95 p-4 shadow-2xl shadow-cyan-900/30 max-w-full"
        style={tooltipStyle}
      >
        <div className={arrowClass.wrapper}>
          <div className={arrowClass.arrow} />
        </div>
...
function DemoPage() {
  ...
  const tourSteps = useMemo(() => ([
    {
      ...
      description: "This is where users see their avatar, name, status, and quick controls like logout and sound.",
      targetRef: profileRef,
      placement: "right",
    },
    {
      title: "Chats And Contacts",
      description: "This panel contains chat threads and contacts. In the real app, all users and conversations are loaded here.",
      targetRef: listRef,
      placement: "right",
    },
    {
      title: "Conversation Area",
      description: "Messages appear here with date grouping, left/right alignment, and media previews.",
      targetRef: messageRef,
      placement: "top",
    },
    {
      title: "Message Composer",
      description: "Type text, attach images, and send messages from this input bar.",
      targetRef: inputRef,
      placement: "top",
    },
  ]), [])
  ...
  useEffect(() => {
    ...
    const updateRect = () => {
      ...
      const currentTarget = tourSteps[tourStep]?.targetRef?.current
      ...
      setContainerSize({ width: containerBox.width, height: containerBox.height })
      ...
      setTargetRect({
        ...
        right: targetBox.right - containerBox.left,
        bottom: targetBox.bottom - containerBox.top,
      })
    }
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full max-w-6xl h-[calc(100vh-2rem)] max-h-[800px] min-h-[560px]"
    >
      <div className={`w-full h-full ${isTourOpen ? "pointer-events-none" : ""}`}>
        <BorderAnimatedContainer>
          ...
          <div className="flex-1 flex flex-col bg-slate-900/50 backdrop-blur-sm">
            <DemoChatHeader />
            <DemoBanner />
            <div ref={messageRef} className="flex-1 min-h-0">
              <DemoMessageArea />
            </div>
            <div ref={inputRef}>
              <DemoMessageInput />
      ...
      {isTourOpen && (
        <DemoTourOverlay
          ...
          placement={tourSteps[tourStep].placement}
